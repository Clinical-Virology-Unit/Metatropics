...
**[`fastq.csv`](fastq.csv)** — for runs that start from **FASTQ** reads: `barcode` is the **full path** to that sample’s read file (typically `.fastq`). To **auto-generate** a samplesheet CSV from a folder of FASTQ files, follow the **`--input`** note in **[`all_options.md`](all_options.md)**.
